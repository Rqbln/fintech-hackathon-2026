  ...
  return (
    <div className={cn("border-l-2 pl-3 py-2 space-y-1.5", borderColor)}>
      ...
      {finding.gap_description && (
        <p className="text-[11px] text-red-600/80 italic">{finding.gap_description}</p>
      )}
          ...
          {finding.evidence_spans.slice(0, 2).map((span, i) => (
            <div key={i} className="group">
              <blockquote className="border-l border-slate-300 pl-2 text-[11px] text-slate-500 italic">
                "{span.text}"
                {span.page > 0 && (
                  <span className="not-italic text-slate-400 ml-1">p.{span.page}</span>
                )}
              </blockquote>
              {onCitationClick && span.document_id && (
                <button
                  onClick={() => onCitationClick(span.document_id, span.page, span.text)}
                  className="flex items-center gap-1 mt-0.5 ml-2 text-[10px] text-indigo-600 hover:text-indigo-700 transition-colors opacity-0 group-hover:opacity-100"
                >
                  <ExternalLink size={9} />
                  See in PDF
                </button>
